...
export const projects: Project[] = [
  ...
  },
  {
    ...
    overview:
      'Sistema creativo IA aplicado en piezas reales: documentación de marca, exploración visual, motion automatizado, video generativo y adaptación de formatos. No es una prueba de herramientas; es un método de producción donde la IA ayuda a llegar más rápido a una base editable y el criterio humano decide qué sirve, qué se ajusta y qué se descarta.',
    role: 'Dirección creativa y automatización: documentación de marca en Obsidian, Claude Code + Remotion + After Effects para reels de paid media, Codex + Gemini Image para carruseles y editorial de producto, y dirección de arte en Gemini Flow e Higgsfield para video generativo.',
    challenge:
      'El reto no era usar IA por novedad, sino integrarla sin romper la identidad de cada marca. Cada flujo conserva un punto de intervención humana: leer guidelines antes de generar, seleccionar cortes, dirigir actuación en video, retocar iluminación y decidir cuándo usar stock en vez de fondos IA para que el resultado se vea más natural.',
    process:
      'El sistema se divide en cinco flujos, los mismos documentados en la página IA: documentación de marca, motion, imagen generativa, video generativo y estático. Para motion, el brief entra a Claude Code y sale una composición Remotion con timing y estructura de cortes; después reviso, selecciono y cierro el entregable en After Effects. Para piezas estáticas, Codex y Gemini ayudan a explorar componentes visuales desde el brief. Para video generativo, Gemini Flow e Higgsfield se usan con dirección de arte desde el prompt inicial hasta las correcciones de actuación.',
    result:
      'Workflow documentado que reduce trabajo repetitivo y acelera la primera base editable sin quitar revisión creativa. Incluye reels producidos con la skill /reel, piezas de video generativo con dirección de actuación, posts editoriales creados con imagen generativa y un flyer World Cup 2026 para KOP adaptado a 5 formatos con script de Photoshop. Para ver el sistema completo, la página IA muestra la lógica de trabajo detrás de estos casos.',
    gallery: [],
    campaigns: [
      {
